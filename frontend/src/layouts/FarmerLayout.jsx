...
import {
  ...
  FiLogOut, FiMenu, FiX, FiMapPin, FiRadio, FiBell, FiFileText
} from 'react-icons/fi'
...
const farmerNavItems = [
  ...
  { path: ROUTES.FARMER_SENSORS,           label: 'Sensor History',    Icon: FiActivity },
  ...
  { path: ROUTES.FARMER_REPORTS,           label: 'My Reports',         Icon: FiFileText },
]
  ...
  useEffect(() => {
    ...
    api.get('/sensors')
      .then(res => {
        ...
        if (sensors.length > 0) setNodeName(sensors[0].name || sensors[0]._id)
      })
      .catch(() => {})
  }, [user])
        ...
        <div className="space-y-8">
          ...
          <div className="p-3 bg-emerald-50 border border-emerald-100 rounded-xl">
            ...
            <p className="text-xs text-slate-500 flex items-center gap-1 mt-1">
              ...
              {user?.location || 'Afgoye, Somalia'}
            </p>
            {nodeName && (
              <p className="text-xs text-slate-500 flex items-center gap-1 mt-1">
                <FiRadio className="w-3 h-3 text-emerald-500 shrink-0" />
                {nodeName}
              </p>
            )}
          </div>
          ...
          <nav className="space-y-1.5">
            {farmerNavItems.map(({ path, label, Icon }) => (
              <NavLink
                key={path}
                to={path}
                onClick={() => setMobileOpen(false)}
