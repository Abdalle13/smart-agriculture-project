...
function filterByDate(readings, range) {
  const now = new Date()
  if (range === 'today') {
    const midnight = new Date(now.getFullYear(), now.getMonth(), now.getDate())
    return readings.filter(r => new Date(r.timestamp) >= midnight)
  }
  if (range === '7d') {
...
function SensorLogsTab() {
  ...
  const byNode    = activeNode === 'all' ? allReadings : allReadings.filter(r => r.sensorId === activeNode)
  const byDate    = filterByDate(byNode, dateRange)
  const chartData = [...byDate].slice(0, 100).reverse().map(r => ({
    time:  new Date(r.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
    value: r[chartParam] ?? 0,
  }))
  const latestRows  = [...byDate].slice(0, 8)
  const activeParam = PARAMS.find(p => p.key === chartParam)
  const activeNodeName = nodes.find(n => n._id === activeNode)?.name

  const chartValues = byDate.map(r => r[chartParam]).filter(v => v != null)
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs border-collapse">
              ...
              <tbody className="divide-y divide-slate-50">
                {latestRows.map((r, i) => {
                  const nodeName = nodes.find(n => n._id === r.sensorId)?.name || r.sensorId
                  return (
...
export default function AdminDataMonitoring() {
  const [readingsCnt, setReadingsCnt] = useState(null)

  useEffect(() => {
    api.get('/sensors/readings/all')
      .then(res => setReadingsCnt(res.data.data?.length ?? 0))
      .catch(() => {})
  }, [])
          ...
          <p className="text-slate-400 text-sm mt-0.5">
            ...
            {readingsCnt != null && (
              <span className="ml-2 inline-flex items-center gap-1 text-[11px] font-bold text-slate-500 bg-slate-100 border border-slate-200 px-2 py-0.5 rounded-lg">
                <FiDatabase size={10} /> {readingsCnt} readings
              </span>
            )}
          </p>
        </div>
